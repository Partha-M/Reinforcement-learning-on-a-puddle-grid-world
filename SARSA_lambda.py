...
for k in range(loop_times):
    ...
    for i in range(episode):
        ...
        if np.random.random() < 1 - epsilon:
            action = np.argmax(Q[state, :])                    # with 1-epsilon probability choose maximizing action
        else:
...
sio.savemat('SARSA_lambda_0_5_A.mat',{'time_steps': x, 'average_rewards':avg_reward_in_episodes,'average_steps':avg_steps_in_episodes})
plt.subplot(2, 1, 1)
